...
The user runs `aws configure` (or `aws sso login`) once on their machine. After that, `./scripts/deploy.sh <stage>` works without any credential input.
...
- After deploy, the key value is emitted as a CDK output (`ApiKey`) and printed by `deploy.sh`
...
├── scripts/
│   ├── deploy.sh                # ./scripts/deploy.sh <stage>
│   ├── destroy.sh               # ./scripts/destroy.sh <stage>
...
`./scripts/deploy.sh <stage>` does:

1. Validate `<stage>` is `prod` or `dev`
2. Load `deploy.yaml`, validate with Pydantic, apply stage overrides
3. Generate `frontend/config.js` with the resolved API URL for this stage
4. For EC2 path: zip `backend/app` and stage it for upload
5. Run `cdk deploy <project>-<stage> --require-approval never`
6. After deploy, print the frontend URL, API URL, and any API key (if generated)

`./scripts/destroy.sh <stage>` runs `cdk destroy <project>-<stage>` after confirmation.

`./scripts/dev-server.sh` does:
1. Copy `frontend/config.local.js` → `frontend/config.js`
2. Start backend: `cd backend/api && uvicorn handler:app --reload --port 8000` (or backend/app for EC2 layout)
...
- AWS CLI configured with credentials for `aws_account`
...
1. With a fresh clone of the template and a valid `deploy.yaml` (Lambda variant), `./scripts/deploy.sh dev` produces a working dev environment at `<project>.dev.<base_domain>` with the API at `api.<project>.dev.<base_domain>`.
...
4. `./scripts/deploy.sh prod` deploys to the prod URLs without affecting the dev stack.
5. `./scripts/dev-server.sh` runs the same frontend and backend locally with `localhost:3000` → `localhost:8000`, no AWS calls required.
6. `./scripts/destroy.sh dev` cleanly tears down the dev stack.
